...
var Manager = exports.Manager = function (options) {  
  ...
  this.growl = options.growl === undefined ? false : options.growl;
  ...
  if(this.growl) {
    self.use(assembly.plugins.growler);
  }
};
...
Manager.prototype.addToQueue = function(src, options, callback){
  ...
  this.processQueue.push({src: src, options : options}, function (err, data) {
    if(err && err.skipped) {
      ...
      self.emit('compile_error', err, src, options);
    }
...
Manager.prototype.writeConfig = function() {
  var self = this;

  var paths = Object.keys(this.config.get('assets')).reduce(function(memo, asset){
  ...
  var out = this.core.prettyPrint(utile.format(template, paths));

  this.emit('write', path.join(this.core.jsRoot, "asset-manifest.js"), out);
};
